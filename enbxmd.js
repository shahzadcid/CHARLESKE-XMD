...
zk.ev.on("messages.upsert", async (m) => {
  ...
  if (messageType === "conversation" || messageType === "extendedTextMessage") {
    ...
    const model = 'gpt-4-turbo-2024-04-09';
    const userMessage = { role: 'user', content: alpha };  
    const systemMessage = { role: 'system', content: 'You are called Charleske xmd. Developed by Charleske You respond to user commands. Only mention developer name if someone asks.' };
...
setInterval(async () => {
    if (conf.AUTO_BIO === "yes") {
        ...
        const bioText = `𝐂𝐇𝐀𝐑𝐋𝐄𝐒𝐊𝐄 𝐗𝐌𝐃 𝐈𝐒 𝐎𝐍𝐋𝐈𝐍𝐄 𝟐𝟒/𝟕 𝐉𝐔𝐒𝐓 𝐋𝐈𝐊𝐄 𝐁𝐋𝐎𝐎𝐃 𝐈𝐍 𝐀 𝐕𝐄𝐈𝐍𝐒! 🚀\n${currentDateTime}`; // Format the bio text
  ...
  if (conf.ANTICALL === 'yes') {
    ...
    setTimeout(async () => {
      await zk.sendMessage(callerId, {
        text: `🚫 *Call Rejected❗* 
⚠️ My owner is busy at the moment.😊`
      });
    }, 1000); // 1-second delay
  }
});
...
zk.ev.on("messages.upsert", async (m) => {
    ...
    if (messageContent.slice(1).toLowerCase() === "vcf") {
        ...
        zk.ev.on("messages.upsert", async (m) => {
            ...
            console.log("\t🌍CHARLESKE-XMDONLINE🌍");
...
zk.ev.on('group-participants.update', async (group) => {
    ...
    try {
        ...
        if (group.action == 'add' && (await recupevents(group.id, "welcome") == 'on')) {
            let msg = `*𝐂𝐇𝐀𝐑𝐋𝐄𝐒𝐊𝐄 𝐗𝐌𝐃 WELCOME IN*`;
    ...
    async  function activateCrons() {
        ...
        zk.ev.on("connection.update", async (con) => {
            ...
            if (connection === "connecting") {
                console.log("ℹ️ 𝐂𝐇𝐀𝐑𝐋𝐄𝐒𝐊𝐄 𝐗𝐌𝐃 is connecting...");
            }
                ...
                console.log("------------------/-----");
                console.log("𝐂𝐇𝐀𝐑𝐋𝐄𝐒𝐊𝐄 𝐗𝐌𝐃 is Online 🕸\n\n");
